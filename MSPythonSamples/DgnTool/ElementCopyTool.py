...
class ElementCopyTool(DgnElementSetTool):
    ...
    def __init__(self, toolId):
        """
        Initialize the ElementCopy tool.
    ...
    def _SetupForModify(self, ev, isDynamics):
        ...
        self.m_dynamic = isDynamics

        return True

    def _OnElementModify(self, eeh):
        """
        Apply the transform previously computed in _SetupForModify to the copy element.
        ...
        ret = eeh.GetHandler(eMISSING_HANDLER_PERMISSION_Transform).ApplyTransform(eeh, tInfo)

        if not self.m_dynamic:
            copier = ElementCopyContext(ISessionMgr.ActiveDgnModelRef)
            ret = copier.DoCopy(eeh)
            tInfoR = TransformInfo(self.m_transformr)
            eeh.GetHandler(eMISSING_HANDLER_PERMISSION_Transform).ApplyTransform(eeh, tInfoR)
    ...
    def _OnRestartTool(self):
        ...
        """
        ElementCopyTool.s_instance = None   # Clear instance reference
    ...
    def InstallNewInstance(toolId):
        ...
        tool.InstallTool()
        ElementCopyTool.s_instance = tool   # Keep instance reference

if __name__ == "__main__":
    SelectionSetManager.GetManager().EmptyAll() # Clear selection
    ElementCopyTool.InstallNewInstance(1)
