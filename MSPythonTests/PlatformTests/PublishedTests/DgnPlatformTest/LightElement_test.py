...
def createLightElement1 (setTypeOnly):
    ...
    result.SetShadowType (AdvancedLight.SHADOWTYPE_RayTrace)
    ...
    result.SetShadowQuality (LightElement.SHADOWQUALITY_Sharp)
    ...
    result.GetMaps ().AddMap (LightMap.MAPTYPE_Color).SetIsEnabled (False)
    result.GetMaps ().AddMap (LightMap.MAPTYPE_VolumetricDensity).SetIsEnabled (False)
    result.GetMaps ().AddMap (LightMap.MAPTYPE_VolumetricScatterColor).SetIsEnabled (False)
    result.GetMaps ().AddMap (LightMap.MAPTYPE_DiffuseAmount).SetIsEnabled (False)
    result.GetMaps ().AddMap (LightMap.MAPTYPE_SpecularAmount).SetIsEnabled (False)
    result.GetMaps ().AddMap (LightMap.MAPTYPE_ShadowColor).SetIsEnabled (False)
    return result
...
def createLightElement2 (setTypeOnly):
    ...
    result.SetCastsShadows (True)
    result.SetShadowQuality (LightElement.SHADOWQUALITY_SoftMedium)
...
def createLightElement3 (setTypeOnly) :
    ...
    map = result.GetMaps ().AddMap (LightMap.MAPTYPE_VolumetricScatterColor)
    map.SetIsEnabled (True)
    map.SetStyle (LightMap.MAPSTYLE_Image)
    ...
    map.SetHorizontalWrap (LightMap.WRAPMODE_Mirror)
    map.SetVerticalWrap (LightMap.WRAPMODE_Edge)
...
def createLightElement4 (setTypeOnly) :
    ...
    map = result.GetMaps ().AddMap (LightMap.MAPTYPE_Color)
    map.SetIsEnabled (True)
    map.SetStyle (LightMap.MAPSTYLE_Procedure)
    # map.SetUnits (MAPUNITS_Meters)
    map.GetSize ().Init (1.0, 1.0, 1.0)
    map.GetOffset ().Init (0.0, 0.0, 0.0)
    map.SetAngleInRadians (0.0)
    map.SetHorizontalWrap (LightMap.WRAPMODE_Mirror)
    map.SetVerticalWrap (LightMap.WRAPMODE_Repeat)
    # map.SetMode (MAPMODE_Spherical)

    # LxoProcedure is not exposed
    # constant = map.AddLxoProcedure (LxoProcedure.PROCEDURETYPE_Constant)
    # constant.GetColor ().red = 159.0 / 255.0
    # constant.GetColor ().green = 57.0 / 255.0
    # constant.GetColor ().blue = 121.0 / 255.0
    ...
    map.GetOffset ().Init (0.0, 0.0, 0.0)
    map.SetAngleInRadians (0.0)
    map.SetHorizontalWrap (LightMap.WRAPMODE_Repeat)
    map.SetVerticalWrap (LightMap.WRAPMODE_Repeat)
    # map.SetMode (MAPMODE_FrontProject)

    # LxoProcedure is not exposed
    # dot = map.AddLxoProcedure (LxoProcedure.PROCEDURETYPE_Dot)
    # dot.GetDotColor ().red = 79.0 / 255.0
    # dot.GetDotColor ().green = 184.0 / 255.0
    # dot.GetDotColor ().blue = 159.0 / 255.0
    # dot.GetFillerColor ().red = 13.0 / 255.0
    # dot.GetFillerColor ().green = 18.0 / 255.0
    # dot.GetFillerColor ().blue = 82.0 / 255.0
...
def createLightElement6 (setTypeOnly) :
    ...
    result.SetShadowType (AdvancedLight.SHADOWTYPE_DeepShadowMap)
    ...
    result.SetCastsShadows (True)
    result.SetShadowQuality (LightElement.SHADOWQUALITY_SoftCoarse)
    result.GetShadowColor ().red = result.GetShadowColor ().green = result.GetShadowColor ().blue = 0.0
    ...
    map = result.GetMaps ().AddMap (LightMap.MAPTYPE_ShadowColor)
    map.SetIsEnabled (True)
    map.SetStyle (LightMap.MAPSTYLE_Image)
    ...
    map.SetHorizontalWrap (LightMap.WRAPMODE_Edge)
    map.SetVerticalWrap (LightMap.WRAPMODE_Mirror)
    # map.SetMode (MAPMODE_Planar)
    map.SetFileName ("brick06.jpg")

    map = result.GetMaps ().AddMap (LightMap.MAPTYPE_VolumetricDensity)
    map.SetIsEnabled (True)
    map.SetStyle (LightMap.MAPSTYLE_Image)
    # map.SetUnits (MAPUNITS_Meters)
    map.GetSize ().Init (1.0, 1.0, 1.0)
...
def createLightElement7 (setTypeOnly) :
    ...
    result.SetVolumeAffectCaustics (100.0)
    result.SetCastsShadows (True)
    result.SetShadowQuality (LightElement.SHADOWQUALITY_SoftVeryFine)
    ...
    map = result.GetMaps ().AddMap (LightMap.MAPTYPE_DiffuseAmount)
    map.SetIsEnabled (True)
    map.SetStyle (LightMap.MAPSTYLE_Procedure)
    # map.SetUnits (MAPUNITS_Meters)
    ...
    map.SetAngleInRadians (0.0)
    map.SetHorizontalWrap (LightMap.WRAPMODE_Reset)
    map.SetVerticalWrap (LightMap.WRAPMODE_Reset)
    # map.SetMode (MAPMODE_Solid)

    #LxoProcedure is not exposed
    # lxoProcedure = map.AddLxoProcedure (LxoProcedure.PROCEDURETYPE_Checker)
    # lxoProcedure.SetValue1 (22.641509)
    # lxoProcedure.SetValue2 (79.245283)
    # lxoProcedure.SetCheckerType (LxoCheckerProcedure.CHECKERTYPE_Cube)
    # lxoProcedure.SetTransitionWidth (37.735849)
    # lxoProcedure.SetBias (71.698113)
    # lxoProcedure.SetGain (32.075472)
    return result
...
def test_LoadAndSaveSetup(initDgnPlatformHost, loadDgnFile):
    ...
    # assert light1.Save () == SUCCESS
    # assert light2.Save () == SUCCESS
    # assert light3.Save () == SUCCESS
    # assert light4.Save () == SUCCESS
    # assert light6.Save () == SUCCESS
    # assert light7.Save () == SUCCESS
    ...
    verts.append(DPoint3d(500,500,0))
    verts.append(DPoint3d(-500,500,0))
    assert ShapeHandler.CreateShapeElement (areaShapeEEh, None, verts, True, ret[0]) == SUCCESS
    assert ShapeHandler.CreateShapeElement (openSkyShapeEEh, None, verts, True, ret[0]) == SUCCESS
    ...
    # assert areaLight.SaveNewLight (areaShapeEEh) == SUCCESS
    # assert skyLight.SaveNewLight (openSkyShapeEEh) == SUCCESS
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_LightSetupSynch(initDgnPlatformHost, loadDgnFile):
    ret = loadDgnFile.CreateNewModel ("Test", DgnModelType.Normal, False)
    light1 = createLightElement1 (False)
    light2 = createLightElement2 (False)
    light3 = createLightElement3 (False)
