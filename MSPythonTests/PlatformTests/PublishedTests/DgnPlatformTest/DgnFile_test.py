...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_FillSectionsInModel(initDgnPlatformHost, loadDgnFile, createTempDgnFileWithSeed):
...
def test_GetLoadedModelsCollection(initDgnPlatformHost, loadDgnFile, createTempDgnFileWithSeed):
    fileSrc = os.environ['MSPYTEMPDATA']+ os.path.basename(__file__).split('.')[0] + str(sys._getframe().f_lineno)+'.dgn'
    srcDgnFile = createTempDgnFileWithSeed (fileSrc, loadDgnFile)
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_DeleteModel(initDgnPlatformHost, loadDgnFile): 
    name= "Test Model"
    model1 = loadDgnFile.CreateNewModel (name, DgnModelType.eNormal, False)
    assert None != model1
    loadDgnFile.DeleteModel(model1[0])
    assert loadDgnFile.FindModelIdByName(name) == INVALID_MODELID
...
def test_DoSaveAs(removeTempCreatedFile, initDgnPlatformHost, loadDgnFile): 
    filenamesave = WString(os.environ['MSPYTESTDATA']+"DgnFile_DoSaveAsTest.dgn")
...
def test_DoSaveTo(removeTempCreatedFile, initDgnPlatformHost, loadDgnFile): 
    filenamesave = WString(os.environ['MSPYTESTDATA']+"DgnFile_DoSaveToTest.dgn")
...
def test_SetModelBackgroundColor_Design(initDgnPlatformHost,loadDgnFile,createTempDgnFileWithSeed):
    fileSrc = os.environ['MSPYTEMPDATA']+ os.path.basename(__file__).split('.')[0] + str(sys._getframe().f_lineno)+'.dgn'
    srcDgnFile = createTempDgnFileWithSeed (fileSrc, loadDgnFile)
