...
def EndTest():
    ...
    BeFileName.BeDeleteFile(str(dataDir+TEMP_FILE_NAME))
    BeFileName.BeDeleteFile(str(dataDir+TEMP_FILE2_NAME))

def OpenTempFile(dgnFile, mode = DgnFileOpenMode.eReadWrite):
    dataDir = os.environ['MSPYTESTDATA']
    m_file = OpenFile0(str(dataDir+TEMP_FILE_NAME), mode, dgnFile)
    return m_file
...
def CreateTempFile(dgnFile):
    dataDir = os.environ['MSPYTESTDATA']
    BeFileName.BeDeleteFile(str(dataDir+TEMP_FILE_NAME))
    tempDgnFile = OpenSeedFile(dgnFile)
    assert tempDgnFile.DoSaveAs(DgnDocument.CreateForLocalFile(str(dataDir+TEMP_FILE_NAME))) == BentleyStatus.eSUCCESS
    tempDgnFile.FillDictionaryModel()
    return tempDgnFile
...
def test_SetLineStyle(initDgnPlatformHost, loadDgnFile):
    ...
    assert level.SetOverrideLineStyle (lsDef, None, loadDgnFile)
    assert level.GetOverrideLineStyle ().GetStyle () == lsNum
    assert level.GetOverrideLineStyle ().GetStyleParams() == None

    assert level.SetByLevelLineStyle (lsDef, None, loadDgnFile)
...
def DeleteOtherLevel(saveTheChange, m_file, newLevelname):
    ...
    tempDgnFile1 = OpenTempFile(m_file)
