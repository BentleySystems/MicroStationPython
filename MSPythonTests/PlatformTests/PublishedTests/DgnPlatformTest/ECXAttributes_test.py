...
def test_FindECInstances(initDgnPlatformHost, loadDgnFile):
    global mgr
    ret = loadDgnFile.CreateNewModel ("Test", DgnModelType.eNormal, False)
    schema = Create_schema()
    mgr = DgnECManager.GetManager()
    mgr.ImportSchema (schema[1], loadDgnFile)
    ...
    widgetEnabler = mgr.ObtainInstanceEnablerByName ("ECXAttributesTest", "Widget", loadDgnFile)
    gadgetEnabler = mgr.ObtainInstanceEnablerByName ("ECXAttributesTest", "Gadget", loadDgnFile)
    ...
    fileScope = FindInstancesScope.CreateScope (loadDgnFile, scopeOption)
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_CreateECInstances(initDgnPlatformHost, loadDgnFile):
    global mgr
    ret = loadDgnFile.CreateNewModel ("Test", DgnModelType.eNormal, False)
    schema = Create_schema()
    mgr = DgnECManager.GetManager()
    mgr.ImportSchema (schema[1], loadDgnFile)
    enabler = mgr.ObtainInstanceEnablerByName ("ECXAttributesTest", "Gadget", loadDgnFile)
    assert enabler != None
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_Persistence(initDgnPlatformHost, loadDgnFile):
    global mgr
    ret = loadDgnFile.CreateNewModel ("Test", DgnModelType.eNormal, False)
    schema = Create_schema()
    mgr = DgnECManager.GetManager()
    ...
    eeh = EditElementHandle()
    CreateHostElement (eeh, True, ret)
    enabler = mgr.ObtainInstanceEnablerByName ("ECXAttributesTest", "Widget", loadDgnFile)
    wip = enabler.GetSharedWipInstance()
