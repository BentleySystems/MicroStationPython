...
def test_GetItemTypeByInternalName(initDgnPlatformHost, loadDgnFile, createTempDgnFileFromSeed):
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_GetPropertyByInternalName(initDgnPlatformHost, loadDgnFile, createTempDgnFileFromSeed):
    srcDgnFile = createTempDgnFileFromSeed (loadDgnFile)
    itemtypeLib = ItemTypeLibrary("TestLibrary", srcDgnFile)
    itpFirst = itemtypeLib.AddItemType("Test Type")
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_GetCustomTypeByInternalName(initDgnPlatformHost, loadDgnFile, createTempDgnFileFromSeed):
    srcDgnFile = createTempDgnFileFromSeed (loadDgnFile)
    itemtypeLib = ItemTypeLibrary("TestLibrary", srcDgnFile)
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_DefaultExpression_StringTest(initDgnPlatformHost, loadDgnFile, createTempDgnFileFromSeed):
    srcDgnFile = createTempDgnFileFromSeed (loadDgnFile)
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_DefaultExpression_Point(initDgnPlatformHost, loadDgnFile, createTempDgnFileFromSeed):
    srcDgnFile = createTempDgnFileFromSeed (loadDgnFile)
    ret = srcDgnFile.CreateNewModel ("Test", DgnModelType.eNormal, False)
