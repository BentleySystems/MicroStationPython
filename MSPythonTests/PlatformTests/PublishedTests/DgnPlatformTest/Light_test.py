...
def validateSetup (expectedSetup, testSetup):
    ...
    assert round(expectedSetup.GetAdaptationLevelForView ()) == round(testSetup.GetAdaptationLevelForView ())
    ...
    assert expectedSetup.GetFattalColorSaturation () == testSetup.GetFattalColorSaturation ()
    assert expectedSetup.GetFattalAttenuation () == testSetup.GetFattalAttenuation ()
...
def createExpectedSetup1 ():
    ...
    result.SetBrightnessMultiplierForView (3.0)
    ...
    solarLight.SetSolarType (SolarLight.SOLARTYPE_TimeLocation)
    ...
    solarLight.SetShadowType (AdvancedLight.SHADOWTYPE_RayTrace)
    ...
    solarLight.SetShadowQuality (Light.SHADOWQUALITY_Sharp)
    solarLight.SetTurbidity (3.0)
    solarLight.SetIsColorPhysicallyBased (True)

    solarLight.GetMaps ().AddMap (LightMap.MAPTYPE_Color).SetIsEnabled (False)
    solarLight.GetMaps ().AddMap (LightMap.MAPTYPE_DiffuseAmount).SetIsEnabled (False)
    solarLight.GetMaps ().AddMap (LightMap.MAPTYPE_ShadowColor).SetIsEnabled (False)
    solarLight.GetMaps ().AddMap (LightMap.MAPTYPE_SpecularAmount).SetIsEnabled (False)

    map = solarLight.GetMaps ().AddMap (LightMap.MAPTYPE_VolumetricScatterColor)
    map.SetIsEnabled (True)
    map.SetFileName ("BRICK02.jpg")

    map = solarLight.GetMaps ().AddMap (LightMap.MAPTYPE_VolumetricDensity)
    map.SetIsEnabled (True)
    ...
    skyLight.SetShadowQuality (Light.SHADOWQUALITY_SoftMedium)
...
@pytest.mark.parametrize('fileName', ['LightSetup.dgn'])
def test_DeleteSetup(initDgnPlatformHost, loadDgnFile):
    modelID = loadDgnFile.GetDefaultModelId()
    ret = loadDgnFile.LoadRootModelById(modelID)
    dgnModel = ret[0]
    assert dgnModel is not None
