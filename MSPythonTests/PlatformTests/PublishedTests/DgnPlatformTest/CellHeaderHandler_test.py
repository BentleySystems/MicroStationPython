...
def test_CreateCellElement(initDgnPlatformHost, loadDgnFile, createTempDgnFileFromSeed):
...
@pytest.mark.parametrize('fileName', ['3dMetricGeneral.dgn'])  
def test_InsertChildPreserveExistingElementIds(initDgnPlatformHost, loadDgnFile, createTempDgnFileFromSeed):
    srcDgnFile = createTempDgnFileFromSeed (loadDgnFile)
    ret = srcDgnFile.CreateNewModel ("Test", DgnModelType.eNormal, True)
    NormalCellHeaderHandler.CreateOrphanCellElement (m_eeh, "test", ret[0].Is3d(), ret[0])
