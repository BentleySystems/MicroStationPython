...
def test_Create(initDgnPlatformHost, loadDgnFile):
    ...
    if BentleyStatus.eSUCCESS != DimensionStyle.Delete(testName, loadDgnFile):
        assert False
...
@pytest.mark.parametrize('fileName', ['2dMetricGeneral.dgn'])
def test_AddBlankNameShouldFail(initDgnPlatformHost, loadDgnFile):
    testStyle = DgnTextStyle("AwesomeTextStyle", loadDgnFile)
    dimStyle = DimensionStyle("AwesomeDimStyle", loadDgnFile)
 
    assert (BentleyStatus.eSUCCESS == dimStyle.Add(loadDgnFile))
    assert (BentleyStatus.eSUCCESS == testStyle.Add(loadDgnFile))

    textStyle = DgnTextStyle("", loadDgnFile)
    dimStyle = DimensionStyle("", loadDgnFile)

    assert BentleyStatus.eERROR == dimStyle.Add(loadDgnFile)
    assert BentleyStatus.eERROR == testStyle.Add(loadDgnFile)
